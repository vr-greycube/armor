...
frappe.ui.form.on('Consumed Materials CT', {
  product_bundle_item_code: function (frm,cdt,cdn) {
    let row=locals[cdt][cdn]
    if (row.product_bundle_item_code) {
      frappe.db.get_value('Item', row.product_bundle_item_code, 'item_name')
      .then(r => {
          let item_name=r.message.item_name
          if (item_name) {
            row.product_bundle_item_name=item_name
            frm.refresh_field('consumed_materials')
          }
      })      
