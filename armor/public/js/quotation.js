...
frappe.ui.form.on(cur_frm.doctype, {
  ...
  validate:function (frm) {
    if (frm.doc.quotation_to==undefined || frm.doc.quotation_to!='Lead') {
      frappe.throw({message:__("Please select 'Quotation To' field value as Lead."),title:__("Quotation To error." )})
    }
    if (frm.doc.party_name==undefined || frm.doc.party_name =='') {
      frappe.throw({message:__("Please select lead."),title:__("Lead value is missing." )})
    }    
  },
  customer_name: function (frm) {
    ...
    .then(r => {
        let customer_group=r.message.customer_group
        if (customer_group) {
          frappe.db.get_doc('Customer Group', customer_group)
          .then(doc => {
